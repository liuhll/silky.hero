...
COPY Silky.BasicData Silky.BasicData
RUN dotnet restore /src/Silky.BasicData/src/Silky.BasicDataHost/Silky.BasicDataHost.csproj --disable-parallel && \
    dotnet build --no-restore -c Release /src/Silky.BasicData/src/Silky.BasicDataHost/Silky.BasicDataHost.csproj

FROM build AS publish
WORKDIR /src/Silky.BasicData/src/Silky.BasicDataHost
RUN dotnet publish --no-restore -c Release -o /app

FROM base AS final
COPY --from=publish /app .
ENTRYPOINT ["dotnet", "Silky.BasicDataHost.dll"]